...
export function InvestmentDetail({ investment, onBack }: { investment: any, onBack: () => void }) {
  ...
  const handleConfirm = async () => {
    ...
    if (openDialog === 'withdraw') {
      ...
      try {
        const response = await fetch(`http://142.93.208.112:8080/api/user/withdraw/${investment.id}`, {
          method: 'PUT',
          headers: {
      ...
      try {
        const response = await fetch('http://142.93.208.112:8080/api/user/investments/deposit', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
