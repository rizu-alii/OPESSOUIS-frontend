...
export default function UserDashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      setLoading(true);
      try {
        const response = await fetch('http://142.93.208.112:8080/api/user/dashboard', {
          method: 'POST',
          headers: {
    ...
    const fetchTransactions = async () => {
      ...
      try {
        const response = await fetch('http://142.93.208.112:8080/api/user/transactions/dashboard/history', {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
