...
function TotalUsers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setApiMessage(null);
      try {
        const response = await fetch('http://142.93.208.112:8080/api/admin/all-users', {
          method: 'GET',
          headers: {
  ...
  const handleSuspendConfirm = async () => {
    ...
    setApiMessage(null)
    try {
      const response = await fetch(`http://142.93.208.112:8080/api/admin/toggle-enabled/${suspendUser.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleEditSave = async () => {
    ...
    setApiMessage(null)
    try {
      const response = await fetch(`http://142.93.208.112:8080/api/admin/user-update/${editUser.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
          ...
          <Dialog open={profileDialogOpen} onOpenChange={setProfileDialogOpen}>
            {profileDialogOpen && selectedUser && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-sm z-50">
                <div className="bg-white dark:bg-[#18181b] p-6 rounded shadow-lg w-full max-w-md space-y-4 text-gray-900 dark:text-gray-100">
                  ...
                  <div className="flex justify-end gap-2 mt-4">
                    <button className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 dark:text-gray-100 hover:bg-gray-300 dark:hover:bg-gray-600" onClick={() => setProfileDialogOpen(false)}>Close</button>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="bg-white dark:bg-[#18181b] p-6 rounded shadow-lg w-full max-w-md space-y-4 text-gray-900 dark:text-gray-100">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border rounded px-3 py-2 bg-white dark:bg-[#23232a] text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-700"
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex justify-end gap-2 mt-4">
                    <button className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 dark:text-gray-100 hover:bg-gray-300 dark:hover:bg-gray-600" onClick={() => setEditDialogOpen(false)} disabled={editLoading}>Cancel</button>
          ...
          <Dialog open={suspendDialogOpen} onOpenChange={setSuspendDialogOpen}>
            {suspendDialogOpen && suspendUser && suspendAction && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
                <div className="bg-white dark:bg-gray-900 text-black dark:text-white p-6 rounded shadow-lg w-full max-w-sm space-y-4">
                  <h3 className="text-lg font-semibold mb-2">
                    Confirm {suspendAction === 'suspend' ? 'Suspension' : 'Activation'}
                  </h3>
                  <p>
                    Are you sure you want to{' '}
                    {suspendAction === 'suspend' ? 'suspend' : 'activate'} this user?
                  </p>
                  <div className="flex justify-end gap-2">
                    <button
                      className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 dark:text-white hover:bg-gray-300 dark:hover:bg-gray-600"
                    ...
                    <button
                      className={`px-4 py-2 rounded ${suspendAction === 'suspend'
