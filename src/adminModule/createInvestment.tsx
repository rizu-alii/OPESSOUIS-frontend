...
export default function Users() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setApiMessage(null)
    try {
      const response = await fetch('http://142.93.208.112:8080/api/admin/create-investment', {
        method: 'POST',
        headers: {
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://142.93.208.112:8080/api/admin/update-investment/${investmentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://142.93.208.112:8080/api/admin/delete-investment/${investmentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const fetchInvestments = async () => {
    ...
    setApiMessage(null)
    try {
      const response = await fetch('http://142.93.208.112:8080/api/admin/get-investments', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    <SearchProvider>
      <SidebarProvider defaultOpen={defaultOpen}>
        ...
        <div className="ml-auto w-full max-w-full flex h-svh flex-col">
          ...
          <Main>
            {apiMessage && (
              <div className={`mb-4 p-3 rounded mx-auto max-w-2xl text-center ${apiMessage.includes('successfully') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                }`}>
                {apiMessage}
              </div>
            )}
